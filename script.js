...
const DOLAR_HOJE = 5.00; // Cotação média para conversão BRL
...
function atualizarDashboard() {
    ...
    // 2. Cálculo de Valor (Soma em USD e converte para BRL)
...
// FUNÇÃO PARA ATUALIZAR PREÇOS DAS CARTAS ANTIGAS (As 411 cartas)
async function sincronizarPrecos() {
    const btn = document.getElementById('syncBtn');
    if (minhaColecao.length === 0) return;

    btn.disabled = true;
    btn.innerText = "⏳ Consultando API (Aguarde)...";

    try {
        for (let i = 0; i < minhaColecao.length; i++) {
            // Atualiza de 5 em 5 para não ser bloqueado pela API
            ...
            if (i % 10 === 0) btn.innerText = `⏳ Atualizando ${i}/${minhaColecao.length}...`;
        }
        localStorage.setItem('minhaColecao', JSON.stringify(minhaColecao));
        mostrarNotificacao("💰 Valores atualizados com sucesso!");
    } catch (e) {
        mostrarNotificacao("❌ Erro na sincronização.");
    }

    btn.disabled = false;
    btn.innerText = "🔄 Atualizar Preços das 411 Cartas";
    renderizarColecao();
}
...
async function buscarCarta() {
    ...
    grid.innerHTML = "<p>Buscando preços de mercado...</p>";
...
function adicionarColecao(id, nome, imagem, set, num, total, preco) {
    ...
    minhaColecao.push({ id, nome, imagem, set, num, total, preco, dataAdicao: Date.now() });
    localStorage.setItem('minhaColecao', JSON.stringify(minhaColecao));
    renderizarColecao();
...
function renderizarColecao() {
    ...
    grid.innerHTML = "";
    filtradas.forEach(c => {
        const pBRL = (c.preco * DOLAR_HOJE).toFixed(2);
        const div = document.createElement('div');
        div.className = "card-item";
        div.innerHTML = `
            <img src="${c.imagem}">
